...
function fixChangePlural(fn) {
    return function (str) {
        ...
        const index = typeof (matches === null || matches === void 0 ? void 0 : matches.index) === "number"
            ? matches.index + matches[1].length - 1
            : 0;
        const suffixMatches = str.match(/_*$/);
        const suffixIndex = suffixMatches === null || suffixMatches === void 0 ? void 0 : suffixMatches.index;
...
function SimplifyInflectorPlugin(builder, { pgSimpleCollections, pgOmitListSuffix, pgSimplifyPatch = true, pgSimplifyAllRows = true, pgShortPk = true, pgSimplifyMultikeyRelations = true, nodeIdFieldName = "nodeId", }) {
    ...
    function omitListSuffix(entity) {
        ...
        if (tag == null)
            return !!pgOmitListSuffix;
        if (tag !== "include" && tag !== "omit")
            throw new Error(`Unrecognized @listSuffix value "${tag}" on ${entity.kind} "${entity.name}". If @listSuffix is set, it must be "omit" or "include".`);
        return tag === "omit";
    }
    ...
    builder.hook("inflection", (oldInflection) => {
        return Object.assign(Object.assign(Object.assign(Object.assign(Object.assign(Object.assign({}, oldInflection), { 
            /*
             ...
             */
            camelCase: fixCapitalisedPlural(oldInflection.camelCase), upperCamelCase: fixCapitalisedPlural(oldInflection.upperCamelCase), 
            /*
             * Pluralize/singularize only supports single words, so only run
             * on the final segment of a name.
             */
            pluralize: fixChangePlural(oldInflection.pluralize), singularize: fixChangePlural(oldInflection.singularize), distinctPluralize(str) {
            ...
            : null)), { computedColumn(pseudoColumnName, proc, _table) {
                return proc.tags.fieldName
                    ? proc.tags.fieldName +
                        (proc.returnsSet ? ConnectionSuffix(proc) : "")
                    : this.camelCase(pseudoColumnName + (proc.returnsSet ? connectionSuffix(proc) : ""));
            },
            ...
            manyRelationByKeys(detailedKeys, table, foreignTable, constraint) {
                if (constraint.tags.foreignFieldName) {
                    if (constraint.tags.foreignSimpleFieldName) {
                        ...
                        return (constraint.tags.foreignFieldName + ConnectionSuffix(constraint));
                    }
                }
                ...
                return (oldInflection.manyRelationByKeys(detailedKeys, table, foreignTable, constraint) + ConnectionSuffix(constraint));
            },
            manyRelationByKeysSimple(detailedKeys, table, foreignTable, constraint) {
                ...
                return (oldInflection.manyRelationByKeys(detailedKeys, table, foreignTable, constraint) + ListSuffix(constraint));
            },
